...
struct Camera {
	Vector3 position = Vector3(0,   .5,   5.0f); 
 ...
 void RenderOpenGLContext(GLuint shader)
 {
 	...
 	for (int i = 0; i < BRICK_COLS; ++i)
	{
		for (int y = 0; y < BRICK_ROWS; ++y)
		{
 		  ...
 		  model.Scale(bricks[y][i].scale );// 1.5f, .5f, 1.0f); //bricks[BRICK_ROWS][BRICK_COLS].scale ); 
	...
	model.Scale( Paddle.scale);// 1.5f, .5f, 1.0f); //bricks[BRICK_ROWS][BRICK_COLS].scale ); 
