...
- 3D Perspective - Model, View and Projection matrices - calculate the Projection matrix 
- how do we consruct a matrix data structure independent of glm? 
- 
*/
#include <iostream>
#include <stdio.h>
...
// UTILITY CONSTANTS
#define Length(value) ( sizeof(value) / sizeof(value[0] ) ) 
#define PI 3.14
#define DEG2RAD ((PI ) / 180.0f) // IS THIS RIGHT?
...
xTransform_x  yTransform_x zTransform_x Translation x
xTransform_y  yTransform_y zTransform_y Translation y
xTransform_z  yTransform_z zTransform_z Translation z
0             0             0            1
...
/*
 - Construct the orthonormal basis (coordinate space) of the camera. 
 	- Get forward direction (position - forward)
 	- up direction, cross to get right
 	- cross right and forward to get local up
...
// Matrix4 Frustrum(float leftPlane, float rightPlane, float bottomPlane, float topPlane, float n, float f) {
...
/*
Not only are there various ways to setup the frustrum perspective, 
but depending on the API, things might need to map differently
DirectX: 0,1 in NDC

*/ 
 	//Mapping z to -1, 1
// Scaling factor and a translation factor
//
...
void InitOpenGL()
{
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(Cube_Norm_UV), Cube_Norm_UV,  GL_STATIC_DRAW ); //6
 ...
 void RenderOpenGLContext(GLuint shader)
 {
 	...
 	//model.Translate( modelPosition);
 	//model.RotateZAxis(modelPosition.x*30.0f );
 	...
 	//glUniformMatrix4fv(shader_view_id, 1, GL_FALSE,   value_ptr( viewMatrix) );
    ...
    glUniformMatrix4fv(shader_proj_id, 1, GL_FALSE, &proj.m[0]);   //cd);//&proj.m[0]);    value_ptr(projMatrix)
 	//glUniformMatrix4fv(shader_proj_id, 1, GL_FALSE, value_ptr(projMatrix) );
